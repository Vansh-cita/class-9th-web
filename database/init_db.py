...
# Generate fresh hash for '#3795@lgvns'
import bcrypt
admin_hash = bcrypt.hashpw(b'#3795@lgvns', bcrypt.gensalt(rounds=10)).decode()
c.execute(
    'INSERT INTO users (username, password, role_number, school_name, user_id, role) VALUES (?, ?, ?, ?, ?, ?)',
    ('Admin', admin_hash, 'ADMIN001', 'Learning Portal', '#3795@lgvns', 'admin')
)
